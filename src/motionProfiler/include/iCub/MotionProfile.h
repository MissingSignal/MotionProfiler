...
protected:
    ...
    yarp::sig::Vector velocityProfile; //to be removed @luca
    ...
    int reverseSign;                   // integer indicating if the action is performed forward (=1) or backward (=-1)
...
public:
    std::string type;                  // vocab representing the type @Luca
...
protected:
		...
		yarp::sig::Vector ToSave; //here we save the velocities (please remove soon) @luca
		yarp::sig::Vector vel_luca;
public:
    GVPMotionProfile();
    ~GVPMotionProfile();
